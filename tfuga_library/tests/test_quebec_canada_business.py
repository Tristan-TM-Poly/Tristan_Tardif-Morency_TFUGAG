...
def test_quebec_canada_business_manager():
    ...
    assert any(g.status == GateStatus.REVIEW for g in gates)
    ready = CompanyQC("ReadyCo", "corporation", True, True, True, True, True, True, True, True, True, True, True)
    assert not manager.blockers(manager.gates(ready))
